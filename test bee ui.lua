...
║                                                                   ║
║  NEW in v1.6:                                                     ║
║  • Settings: Text Color section (swatches + custom RGB)           ║
║  • Settings: Surface Color section (rows/panels background)       ║
║  • Settings: Tab Color section (active tab color)                 ║
║  • Settings: Button Color section (action buttons color)          ║
║  • Fixed: Settings tab background now follows Background Color    ║
║  • Fixed: GUI Opacity works correctly (no white flash)            ║
║  • Fixed: Close/Min buttons use ASCII X and - (always render)     ║
║  • Fixed: Dropdown arrow uses ASCII v/^ (always render)           ║
╚═══════════════════════════════════════════════════════════════════╝
]]
...
local TextColorPresets = {
    ...
    { Name = "Warm White", Color = Color3.fromRGB(255, 245, 225) },
    { Name = "Yellow",     Color = Color3.fromRGB(255, 220, 100) },
    { Name = "Cyan",       Color = Color3.fromRGB(100, 220, 255) },
    { Name = "Green",      Color = Color3.fromRGB(100, 255, 160) },
    { Name = "Pink",       Color = Color3.fromRGB(255, 160, 200) },
    { Name = "Orange",     Color = Color3.fromRGB(255, 180, 80 ) },
    { Name = "Light Gray", Color = Color3.fromRGB(180, 180, 200) },
    { Name = "Dark",       Color = Color3.fromRGB(20,  20,  25 ) },
    { Name = "Black",      Color = Color3.fromRGB(5,   5,   8  ) },
}

-- Surface color presets (rows, panels)
local SurfaceColorPresets = {
    { Name = "Dark Honey",  Color = Color3.fromRGB(50,  33,  10 ) },
    { Name = "Dark Gray",   Color = Color3.fromRGB(35,  35,  42 ) },
    { Name = "Night",       Color = Color3.fromRGB(20,  20,  35 ) },
    { Name = "Forest",      Color = Color3.fromRGB(15,  35,  22 ) },
    { Name = "Midnight",    Color = Color3.fromRGB(25,  18,  50 ) },
    { Name = "Black",       Color = Color3.fromRGB(12,  12,  15 ) },
    { Name = "Light",       Color = Color3.fromRGB(235, 235, 245) },
    { Name = "White",       Color = Color3.fromRGB(255, 255, 255) },
    { Name = "Sky",         Color = Color3.fromRGB(200, 220, 248) },
    { Name = "Ruby",        Color = Color3.fromRGB(55,  12,  18 ) },
}

-- Tab active color presets
local TabColorPresets = {
    { Name = "Honey",   Color = Color3.fromRGB(210, 145, 20 ) },
    { Name = "Violet",  Color = Color3.fromRGB(120, 80,  220) },
    { Name = "Blue",    Color = Color3.fromRGB(40,  110, 220) },
    { Name = "Green",   Color = Color3.fromRGB(30,  180, 100) },
    { Name = "Pink",    Color = Color3.fromRGB(210, 80,  150) },
    { Name = "Red",     Color = Color3.fromRGB(200, 50,  50 ) },
    { Name = "Teal",    Color = Color3.fromRGB(15,  155, 140) },
    { Name = "Orange",  Color = Color3.fromRGB(220, 120, 40 ) },
    { Name = "White",   Color = Color3.fromRGB(200, 200, 210) },
    { Name = "Gold",    Color = Color3.fromRGB(220, 180, 40 ) },
}

-- Button color presets
local ButtonColorPresets = {
    ...
    { Name = "White",   Color = Color3.fromRGB(220, 220, 230) },
    { Name = "Dark",    Color = Color3.fromRGB(60,  60,  80 ) },
}

-- Lucide fallback — ASCII/Unicode that actually render in Ubuntu/Gotham
...
BeeUI.Themes = {
    Dark = {
        ...
        ButtonColor       = Color3.fromRGB(255, 180, 30 ),
        ButtonColorHover  = Color3.fromRGB(255, 200, 70 ),
    },
    Light = {
        ...
        ButtonColor       = Color3.fromRGB(99,  70,  240),
        ButtonColorHover  = Color3.fromRGB(120, 95,  255),
    },
}
...
function Util.Stroke(p, color, thickness, transparency)
    local s=Instance.new("UIStroke"); s.Color=color or Color3.new(1,1,1)
    s.Thickness=thickness or 1; s.Transparency=transparency or 0
    s.ApplyStrokeMode=Enum.ApplyStrokeMode.Border; s.Parent=p; return s
end
function Util.ListLayout(p, dir, padding, align)
...
function Util.TruncateText(text, font, textSize, maxWidth)
    ...
    local fs=TextService:GetTextSize(text, textSize, font, Vector2.new(9999,9999))
    if fs.X<=maxWidth then return text end
    local ds=TextService:GetTextSize("...", textSize, font, Vector2.new(9999,9999))
    local av=maxWidth-ds.X; local lo,hi,result=1,#text,""
    while lo<=hi do
        local mid=math.floor((lo+hi)/2)
        local sub=text:sub(1,mid)
        local sz=TextService:GetTextSize(sub, textSize, font, Vector2.new(9999,9999))
        if sz.X<=av then result=sub; lo=mid+1 else hi=mid-1 end
    end
    return result.."..."
end
function Util.MakeDraggable(frame, handle)
    ...
    handle.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; dragStart=inp.Position; startPos=frame.Position end
    end)
...
function Util.ClickEffect(btn)
    local ps=nil
    btn.MouseButton1Down:Connect(function() ps=btn.Size; Util.TweenFast(btn,{Size=UDim2.new(ps.X.Scale,ps.X.Offset-2,ps.Y.Scale,ps.Y.Offset-2)},0.08) end)
    btn.MouseButton1Up:Connect(function() if ps then Util.TweenFast(btn,{Size=ps},0.1); ps=nil end end)
    btn.MouseLeave:Connect(function() if ps then Util.TweenFast(btn,{Size=ps},0.1); ps=nil end end)
end

-- ══════════════════════════════════════════
--  GUI OPACITY — правильная реализация
--  Сохраняем оригинальные значения один раз
--  и интерполируем между orig и 1
-- ══════════════════════════════════════════
local function buildTransparencySystem(windowFrame)
    local origValues = nil
        ...
        local function recurse(obj)
            for _, child in ipairs(obj:GetChildren()) do
                ...
                if child:IsA("Frame") or child:IsA("ScrollingFrame") then
                    origValues[child] = {type="bg", val=child.BackgroundTransparency}
                elseif child:IsA("UIStroke") then
                    origValues[child] = {type="stroke", val=child.Transparency}
                end
                recurse(child)
            end
        end
        origValues[windowFrame] = {type="bg", val=windowFrame.BackgroundTransparency}
        recurse(windowFrame)
    end

    return function(alpha)
        -- Собираем оригиналы только один раз после полного построения GUI
        if not origValues then collect() end
        for obj, info in pairs(origValues) do
            if not obj or not obj.Parent then continue end
            -- Интерполируем: 0 = оригинал, 1 = полная прозрачность
            local newVal = info.val + (1 - info.val) * alpha
            pcall(function()
                if info.type=="bg" then
                    obj.BackgroundTransparency = newVal
                elseif info.type=="stroke" then
                    obj.Transparency = newVal
                end
            end)
        end
    end
...
-- ══════════════════════════════════════════
--  SWATCH BUILDER — переиспользуемая функция
--  для построения строки цветных кнопок
-- ══════════════════════════════════════════
local function buildSwatchRow(parent, theme, presets, onSelect)
    local container = Util.Frame(parent, {
    ...
    Util.Padding(container, 10, 10, 10, 10)
    ...
    grid:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        container.Size = UDim2.new(1, 0, 0, grid.AbsoluteContentSize.Y + 20)
    end)

    for i, preset in ipairs(presets) do
        local sw = Util.Button(container, {
        ...
        sw.MouseButton1Click:Connect(function() onSelect(preset.Color) end)
    end
...
-- ══════════════════════════════════════════
--  CUSTOM RGB INPUT — переиспользуемая функция
-- ══════════════════════════════════════════
local function buildRgbInput(parent, scrollFrame, theme, Window, labelText, onSelect)
    local row = Util.Frame(scrollFrame, {
        Name             = "RgbRow_"..labelText,
    ...
    inputBox.FocusLost:Connect(function()
        ...
        if r and g and b then
            local rc = math.clamp(tonumber(r)/255,0,1)
            local gc = math.clamp(tonumber(g)/255,0,1)
            local bc = math.clamp(tonumber(b)/255,0,1)
            onSelect(Color3.new(rc, gc, bc))
        end
    end)
end
...
function BeeUI:CreateWindow(config)
    ...
    local dropOverlay = Util.Frame(screenGui, {
        Name="DropdownOverlay", Size=UDim2.new(1,0,1,0), BackgroundTransparency=1, ZIndex=500,
    })
    ...
    -- Кнопки Close/Min — только ASCII, всегда рендерятся
    local btnClose = Util.Button(titleBar, {
        Name="CloseBtn", Text="X",
    ...
    local tabSidebar = Util.Frame(bodyFrame, {
        Name="TabSidebar", Size=UDim2.new(0,140,1,0),
    ...
    Util.Frame(tabSidebar,{Name="SettingsDivider",Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,-47),BackgroundColor3=theme.Border})
    ...
    Util.Frame(bodyFrame,{Name="SidebarDivider",Size=UDim2.new(0,1,1,0),Position=UDim2.new(0,140,0,0),BackgroundColor3=theme.Border})
    ...
    Window._fontTargets      = {}    -- для смены шрифта
    Window._textTargets      = {}    -- для смены цвета TextPrimary
    Window._surfaceTargets   = {}    -- для смены цвета SurfaceElevated
    Window._tabBtnTargets    = {}    -- все кнопки табов (inactive)
    Window._actionBtnTargets = {}    -- все кнопки AddButton
    Window._settingsTabEntry = nil
    Window._titleLabel       = titleLabel
    Window._subTitleLabel    = subTitleLabel
    -- Текущие цвета (меняются из Settings)
    Window._currentTabActive    = theme.TabActive
    Window._currentTabInactive  = theme.TabInactive
    Window._currentButtonColor  = theme.ButtonColor or theme.Accent
    Window._currentButtonHover  = theme.ButtonColorHover or theme.AccentHover
    Window._currentSurface      = theme.SurfaceElevated
    ...
    local function deactivateEntry(entry)
        ...
        Util.TweenFast(entry.Button,{BackgroundColor3=Window._currentTabInactive},0.35)
        Util.TweenFast(entry.Label,{TextColor3=theme.TabText},0.35)
        local ic=entry.Button:FindFirstChild("IconLabel")
        if ic then Util.TweenFast(ic,{TextColor3=theme.TabText},0.35) end
        if entry.Content then entry.Content.Visible=false end
    end

    local function activateEntry(entry)
        if not entry then return end
        entry.Content.Visible=true
        Util.TweenFast(entry.Button,{BackgroundColor3=Window._currentTabActive},0.35)
        Util.TweenFast(entry.Label,{TextColor3=Color3.fromRGB(255,255,255)},0.35)
        local ic=entry.Button:FindFirstChild("IconLabel")
        if ic then Util.TweenFast(ic,{TextColor3=Color3.fromRGB(255,255,255)},0.35) end
    end
    ...
    local function createTabButton(parent, tabName, tabConfig, isFirst)
        local tabBtn = Util.Button(parent,{
            ...
            BackgroundColor3=isFirst and Window._currentTabActive or Window._currentTabInactive,
        })
        Util.Corner(tabBtn,8)
        -- Регистрируем кнопку таба
        table.insert(Window._tabBtnTargets, {btn=tabBtn, isFirst=isFirst})
        ...
        if tabConfig and tabConfig.Icon then
            ...
            if iconInfo.type=="lucide" then
                local iconLbl=Util.Label(tabBtn,{
                    Name="IconLabel",
                    Text=getLucideChar(iconInfo.name),
                    Font=Enum.Font.GothamBold, TextSize=13,
        ...
        local availW=124-textOffsetX-4
        local tabLabel=Util.Label(tabBtn,{
            Text=Util.TruncateText(tabName,Enum.Font.Ubuntu,13,availW),
    ...
    function Window:AddTab(tabConfig)
        ...
        Util.HoverEffect(tabBtn, isFirst and Window._currentTabActive or Window._currentTabInactive, theme.SurfaceHover)
        ...
        local function makeRow(label, height)
            local row=Util.Frame(scrollFrame,{
                ...
                BackgroundColor3=self._currentSurface,
            })
            Util.Corner(row,10); Util.Stroke(row,theme.Border,1,0)
            -- Регистрируем для смены цвета Surface
            table.insert(self._surfaceTargets, row)
            return row
        end
        ...
        local function makeLabel(parent, text, x, y, w, h, font, size, color)
            ...
            -- Если это TextPrimary — регистрируем для смены цвета текста
            if not color or color==theme.TextPrimary then
                table.insert(Tab._window._textTargets, lbl)
            end
            return lbl
        end
        ...
        local function buildDropdown(parentFrame, options, defaultSelected, onSelect)
            ...
            -- Стрелка — ASCII "v" и "^", всегда рендерятся
            ...
            local totalH=math.max(#options,1)*(ITEM_H+ITEM_PAD)-ITEM_PAD+LIST_PAD*2
            local visH=math.min(math.max(#options,1),MAX_VIS)*(ITEM_H+ITEM_PAD)-ITEM_PAD+LIST_PAD*2
            ...
            for _,opt in ipairs(options) do
                ...
                optBtn.MouseButton1Click:Connect(function()
                    ...
                    for _,c in ipairs(listScroll:GetChildren()) do if c:IsA("TextButton") then c.TextColor3=theme.TextPrimary end end
                    optBtn.TextColor3=theme.Accent; isOpen=false; listOuter.Visible=false
                    arrowLbl.Text="v"; onSelect(opt)
                end)
            end
            dropBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    listOuter.Position=UDim2.new(0,bp.X,0,pY); listOuter.Visible=true; arrowLbl.Text="^"
                else listOuter.Visible=false; arrowLbl.Text="v" end
            end)
            local connInput=UserInputService.InputBegan:Connect(function(inp)
                if isOpen and inp.UserInputType==Enum.UserInputType.MouseButton1 then
                    ...
                    local bP=dropBtn.AbsolutePosition;local bS=dropBtn.AbsoluteSize
                    local inside=mP.X>=lP.X and mP.X<=lP.X+lS.X and mP.Y>=lP.Y and mP.Y<=lP.Y+lS.Y
                    local onBtn=mP.X>=bP.X and mP.X<=bP.X+bS.X and mP.Y>=bP.Y and mP.Y<=bP.Y+bS.Y
                    if not inside and not onBtn then isOpen=false; listOuter.Visible=false; arrowLbl.Text="v" end
                end
            end)
            screenGui.AncestryChanged:Connect(function() if not screenGui.Parent then connInput:Disconnect() end end)
            local obj={}
        ...
        function Tab:AddButton(config)
            ...
            makeLabel(row,Util.TruncateText(config.Label or "Button",Enum.Font.Ubuntu,14,195),14,nil,195,20)
            local btn=Util.Button(row,{
                Name="ActionBtn",
                Text=Util.TruncateText(config.ButtonText or config.Label or "Button",Enum.Font.Ubuntu,13,96),
                Font=self._window._currentFont, TextSize=13,
                TextColor3=Color3.fromRGB(255,255,255),
                BackgroundColor3=self._window._currentButtonColor,
                Size=UDim2.new(0,110,0,30), Position=UDim2.new(1,-122,0.5,-15),
                ClipsDescendants=true,
            })
            Util.Corner(btn,8); Util.Padding(btn,0,7,0,7)
            Util.HoverEffect(btn,self._window._currentButtonColor,self._window._currentButtonHover)
            Util.ClickEffect(btn)
            table.insert(self._window._fontTargets,btn)
            -- Регистрируем для смены цвета кнопки
            table.insert(self._window._actionBtnTargets, btn)
        ...
        function Tab:AddToggle(config)
            ...
            local row=makeRow(config.Label,48)
            makeLabel(row,Util.TruncateText(config.Label or "Toggle",Enum.Font.Ubuntu,14,195),14,nil,220,20)
        ...
        function Tab:AddSlider(config)
            ...
            table.insert(self._window._fontTargets,nameLbl); table.insert(self._window._textTargets,nameLbl)
            ...
            local cc=UserInputService.InputChanged:Connect(function(inp) if dragging and inp.UserInputType==Enum.UserInputType.MouseMovement then upd(inp.Position.X) end end)
            local ce=UserInputService.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end end)
            screenGui.AncestryChanged:Connect(function() if not screenGui.Parent then cc:Disconnect(); ce:Disconnect() end end)
            local obj={}
        ...
        function Tab:AddDropdown(config)
            config=config or {}
            local row=makeRow(config.Label,48)
            makeLabel(row,Util.TruncateText(config.Label or "Dropdown",Enum.Font.Ubuntu,14,195),14,nil,200,20)
        ...
        function Tab:AddInput(config)
            config=config or {}
            local row=makeRow(config.Label,48)
            makeLabel(row,Util.TruncateText(config.Label or "Input",Enum.Font.Ubuntu,14,160),14,nil,160,20)
            local inputBox=Instance.new("TextBox")
            ...
            table.insert(self._window._fontTargets,inputBox); table.insert(self._window._textTargets,inputBox)
        ...
        function Tab:AddColorPicker(config)
            ...
            local row=makeRow(config.Label,48)
            makeLabel(row,Util.TruncateText(config.Label or "Color",Enum.Font.Ubuntu,14,195),14,nil,200,20)
            ...
            local pickerFrame=Util.Frame(Tab._dropOverlay,{Name="ColorPickerPopup",Size=UDim2.new(0,200,0,110),BackgroundColor3=theme.SurfaceElevated,Visible=false,ZIndex=510})
            Util.Corner(pickerFrame,10); Util.Stroke(pickerFrame,theme.BorderAccent,1,0)
            Util.Padding(pickerFrame,10,10,10,10); Util.ListLayout(pickerFrame,Enum.FillDirection.Vertical,6)
            local function makeHsvSlider(parent,label,initVal,onChanged)
                local sf=Util.Frame(parent,{Size=UDim2.new(1,0,0,28),BackgroundTransparency=1})
                Util.Label(sf,{Text=label,Font=Enum.Font.Ubuntu,TextSize=11,TextColor3=theme.TextSecondary,Size=UDim2.new(0,12,1,0),Position=UDim2.new(0,0,0,0)})
                local th=5;local tr=Util.Frame(sf,{Size=UDim2.new(1,-20,0,th),Position=UDim2.new(0,16,0.5,-th/2),BackgroundColor3=theme.SliderTrack})
                Util.Corner(tr,th/2)
                local fl=Util.Frame(tr,{Size=UDim2.new(initVal,0,1,0),BackgroundColor3=theme.SliderFill});Util.Corner(fl,th/2)
                local kS=12;local kn=Util.Frame(tr,{Size=UDim2.new(0,kS,0,kS),Position=UDim2.new(initVal,-kS/2,0.5,-kS/2),BackgroundColor3=Color3.fromRGB(255,255,255),ZIndex=512});Util.Corner(kn,kS/2)
                local dg=false
                tr.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then dg=true end end)
                local cc=UserInputService.InputChanged:Connect(function(inp) if dg and inp.UserInputType==Enum.UserInputType.MouseMovement then local r=math.clamp((inp.Position.X-tr.AbsolutePosition.X)/tr.AbsoluteSize.X,0,1);fl.Size=UDim2.new(r,0,1,0);kn.Position=UDim2.new(r,-kS/2,0.5,-kS/2);onChanged(r) end end)
                local ce=UserInputService.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then dg=false end end)
                screenGui.AncestryChanged:Connect(function() if not screenGui.Parent then cc:Disconnect();ce:Disconnect() end end)
            end
            local function updateColor() color=Color3.fromHSV(h,s,v);preview.BackgroundColor3=color;if config.Callback then pcall(config.Callback,color) end end
            makeHsvSlider(pickerFrame,"H",h,function(r) h=r;updateColor() end)
            makeHsvSlider(pickerFrame,"S",s,function(r) s=r;updateColor() end)
            makeHsvSlider(pickerFrame,"V",v,function(r) v=r;updateColor() end)
            local cb=Util.Button(pickerFrame,{Text="Close",Font=Enum.Font.Ubuntu,TextSize=11,TextColor3=theme.TextPrimary,BackgroundColor3=theme.ControlBg,Size=UDim2.new(1,0,0,22),ZIndex=511});Util.Corner(cb,5)
            cb.MouseButton1Click:Connect(function() pickerOpen=false;pickerFrame.Visible=false end)
            local cz=Util.Button(row,{Text="",BackgroundTransparency=1,Size=UDim2.new(0,44,1,0),Position=UDim2.new(1,-52,0,0)})
            cz.MouseButton1Click:Connect(function() pickerOpen=not pickerOpen;if pickerOpen then local ap=preview.AbsolutePosition;local as=preview.AbsoluteSize;pickerFrame.Position=UDim2.new(0,ap.X-80,0,ap.Y+as.Y+4);pickerFrame.Visible=true else pickerFrame.Visible=false end end)
            local ci=UserInputService.InputBegan:Connect(function(inp) if pickerOpen and inp.UserInputType==Enum.UserInputType.MouseButton1 then local mP=inp.Position;local pP=pickerFrame.AbsolutePosition;local pS=pickerFrame.AbsoluteSize;local inside=mP.X>=pP.X and mP.X<=pP.X+pS.X and mP.Y>=pP.Y and mP.Y<=pP.Y+pS.Y;local cP=cz.AbsolutePosition;local cS=cz.AbsoluteSize;local onBtn=mP.X>=cP.X and mP.X<=cP.X+cS.X and mP.Y>=cP.Y and mP.Y<=cP.Y+cS.Y;if not inside and not onBtn then pickerOpen=false;pickerFrame.Visible=false end end end)
            screenGui.AncestryChanged:Connect(function() if not screenGui.Parent then ci:Disconnect() end end)
            local obj={}; function obj:Set(c) color=c;h,s,v=Color3.toHSV(c);preview.BackgroundColor3=c end; function obj:Get() return color end; return obj
        end
        ...
        function Tab:AddKeybind(config)
            ...
            local row=makeRow(config.Label,48)
            makeLabel(row,Util.TruncateText(config.Label or "Keybind",Enum.Font.Ubuntu,14,195),14,nil,200,20)
            ...
            Util.Corner(kbBtn,8); Util.Stroke(kbBtn,theme.ControlBorder,1,0); table.insert(self._window._fontTargets,kbBtn)
            kbBtn.MouseButton1Click:Connect(function() listening=true;kbBtn.Text="[...]";kbBtn.TextColor3=theme.Warning end)
            local ck=UserInputService.InputBegan:Connect(function(inp,gp)
            ...
            screenGui.AncestryChanged:Connect(function() if not screenGui.Parent then ck:Disconnect() end end)
    ...
    local function buildSettingsTab()
        ...
        -- FIX: фон Settings контента должен совпадать с основным фоном
        local tabContent=Util.Frame(contentArea,{
            Name=tabName.."_Content",Size=UDim2.new(1,0,1,0),
            BackgroundColor3=theme.Background,  -- явно задаём фон
        ...
        -- FIX: при смене фона windowFrame — синхронизируем Settings content
        -- Это делается в секции Background Color ниже через дополнительный вызов
        ...
        Util.HoverEffect(tabBtn,Window._currentTabInactive,theme.SurfaceHover)

        local function sMakeRow(label,height)
            local row=Util.Frame(scrollFrame,{
                Name="Row_"..(label or "Item"),Size=UDim2.new(1,0,0,height or 48),
                BackgroundColor3=Window._currentSurface,
            })
            Util.Corner(row,10); Util.Stroke(row,theme.Border,1,0)
            table.insert(Window._surfaceTargets, row)
            return row
        end
        ...
        do
            ...
            table.insert(Window._fontTargets,lbl); table.insert(Window._textTargets,lbl)
            ...
            for _,opt in ipairs(fontNames) do
                ...
                optBtn.MouseButton1Click:Connect(function()
                    selected=opt; dropLabel.Text=Util.TruncateText(opt,Enum.Font.Ubuntu,13,96)
                    for _,c in ipairs(listScroll:GetChildren()) do if c:IsA("TextButton") then c.TextColor3=theme.TextPrimary end end
                    optBtn.TextColor3=theme.Accent; isOpen=false; listOuter.Visible=false; arrowLbl.Text="v"
            ...
            local cf=UserInputService.InputBegan:Connect(function(inp)
                if isOpen and inp.UserInputType==Enum.UserInputType.MouseButton1 then
                    local mP=inp.Position;local lP=listOuter.AbsolutePosition;local lS=listOuter.AbsoluteSize
                    local bP=dropBtn.AbsolutePosition;local bS=dropBtn.AbsoluteSize
                    local inside=mP.X>=lP.X and mP.X<=lP.X+lS.X and mP.Y>=lP.Y and mP.Y<=lP.Y+lS.Y
            ...
            screenGui.AncestryChanged:Connect(function() if not screenGui.Parent then cf:Disconnect() end end)
        end
        ...
        -- ╚════════════════════════════════════╝
        sSection("Text Color")
        buildSwatchRow(scrollFrame, theme, TextColorPresets, function(color)
            -- Применяем ко всем TextPrimary таргетам
            for _, obj in ipairs(Window._textTargets) do
                if obj and obj.Parent then
                    pcall(function() obj.TextColor3=color end)
                end
            end
            -- Также меняем titleLabel
            Util.TweenFast(titleLabel,{TextColor3=color},0.2)
        end)
        buildRgbInput(nil, scrollFrame, theme, Window, "Custom Text (R,G,B)", function(color)
            for _, obj in ipairs(Window._textTargets) do
                if obj and obj.Parent then
                    pcall(function() obj.TextColor3=color end)
                end
        ...
        buildSwatchRow(scrollFrame, theme, BackgroundPresets, function(color)
            Util.TweenFast(windowFrame,{BackgroundColor3=color},0.3)
            -- FIX: Settings content тоже обновляем
            Util.TweenFast(tabContent,{BackgroundColor3=color},0.3)
            ...
            Util.TweenFast(settingsBtnHolder,{BackgroundColor3=ns},0.3)
        end)
        buildRgbInput(nil, scrollFrame, theme, Window, "Custom BG (R,G,B)", function(color)
            Util.TweenFast(windowFrame,{BackgroundColor3=color},0.3)
            Util.TweenFast(tabContent,{BackgroundColor3=color},0.3)
            local r,g,b=color.R,color.G,color.B
            local bright=0.299*r+0.587*g+0.114*b; local off=bright<0.5 and 0.04 or -0.04
        ...
        -- ╔════════════════════════════════════╗
        -- ║  SURFACE COLOR (строки, панели)    ║
        -- ╚════════════════════════════════════╝
        sSection("Surface Color")
        buildSwatchRow(scrollFrame, theme, SurfaceColorPresets, function(color)
            Window._currentSurface=color
            for _, row in ipairs(Window._surfaceTargets) do
                if row and row.Parent then
                    Util.TweenFast(row,{BackgroundColor3=color},0.25)
                end
            end
        end)
        buildRgbInput(nil, scrollFrame, theme, Window, "Custom Surface (R,G,B)", function(color)
            Window._currentSurface=color
            for _, row in ipairs(Window._surfaceTargets) do
                if row and row.Parent then
                    Util.TweenFast(row,{BackgroundColor3=color},0.25)
        ...
        -- ╚════════════════════════════════════╝
        sSection("Tab Color")
        buildSwatchRow(scrollFrame, theme, TabColorPresets, function(color)
            Window._currentTabActive=color
            -- Обновляем активный таб
            if Window._activeTab then
                Util.TweenFast(Window._activeTab.Button,{BackgroundColor3=color},0.25)
            end
            -- Обновляем AccentLine
            Util.TweenFast(accentLine,{BackgroundColor3=color},0.25)
        end)
        buildRgbInput(nil, scrollFrame, theme, Window, "Custom Tab (R,G,B)", function(color)
            Window._currentTabActive=color
            if Window._activeTab then
                Util.TweenFast(Window._activeTab.Button,{BackgroundColor3=color},0.25)
            end
        ...
        -- ║  BUTTON COLOR                      ║
        -- ╚════════════════════════════════════╝
        sSection("Button Color")
        buildSwatchRow(scrollFrame, theme, ButtonColorPresets, function(color)
            Window._currentButtonColor=color
            -- Hover = чуть светлее
            local r,g,b=color.R,color.G,color.B
            Window._currentButtonHover=Color3.new(math.min(r+0.08,1),math.min(g+0.08,1),math.min(b+0.08,1))
            for _, btn in ipairs(Window._actionBtnTargets) do
                if btn and btn.Parent then
                    Util.TweenFast(btn,{BackgroundColor3=color},0.25)
                    -- Обновляем hover эффект
                    Util.HoverEffect(btn, color, Window._currentButtonHover)
                end
            end
        end)
        buildRgbInput(nil, scrollFrame, theme, Window, "Custom Button (R,G,B)", function(color)
            Window._currentButtonColor=color
            local r,g,b=color.R,color.G,color.B
            Window._currentButtonHover=Color3.new(math.min(r+0.08,1),math.min(g+0.08,1),math.min(b+0.08,1))
            for _, btn in ipairs(Window._actionBtnTargets) do
        ...
        do
            local applyTransparency=buildTransparencySystem(windowFrame)
            ...
            table.insert(Window._fontTargets,nameLbl); table.insert(Window._textTargets,nameLbl)
            ...
            local function upd(ix)
                ...
                -- Вызываем правильную функцию прозрачности
                -- Сначала ждём один кадр чтобы GUI был полностью построен
                applyTransparency(snap/100)
            end
            track.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; upd(inp.Position.X) end end)
            local cc=UserInputService.InputChanged:Connect(function(inp) if dragging and inp.UserInputType==Enum.UserInputType.MouseMovement then upd(inp.Position.X) end end)
            local ce=UserInputService.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then dragging=false end end)
...
function BeeUI:Notify(config)
    ...
    local notify=Util.Frame(holder,{
        Name="Notify_"..tostring(self._notifyCount),Size=UDim2.new(1,0,0,0),
        BackgroundColor3=theme.NotifyBg,ClipsDescendants=true,
        BackgroundTransparency=0,LayoutOrder=self._notifyCount,
    })
    ...
    local pt=Util.Frame(notify,{Size=UDim2.new(1,-8,0,2),Position=UDim2.new(0,4,1,-6),BackgroundColor3=theme.Border})
    Util.Corner(pt,1)
    local pf=Util.Frame(pt,{Size=UDim2.new(1,0,1,0),BackgroundColor3=accentColor}); Util.Corner(pf,1)
    Util.Tween(notify,{Time=0.45,Ease=Enum.EasingStyle.Back,Dir=Enum.EasingDirection.Out},{Size=UDim2.new(1,0,0,76)})
    Util.Tween(pf,TweenInfo.new(duration,Enum.EasingStyle.Linear),{Size=UDim2.new(0,0,1,0)})
    task.delay(duration,function()
        Util.TweenFast(notify,{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1},0.35)
        task.delay(0.35,function() if notify and notify.Parent then notify:Destroy() end end)
    end)
    return notify
end
